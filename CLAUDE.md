...
| `/kits` | `KitBuilder.svelte` | Build/edit Deluge kit XML from card samples (vim-style keys), 16-step drum sequencer with default 808 kit (synthesized drums, always visible) |
...
| `drumSynth.ts` | Synthesized 808 drum sounds (kick, snare, hats, clap, rim, tom, cowbell) via `OfflineAudioContext`. `render808Buffers()` pre-renders all 8 into `AudioBuffer`s; `create808Kit()` returns a Kit with named rows; `DEFAULT_808_PATTERN` is a starter beat |
| `sequencerAudio.ts` | 16-step drum sequencer engine for `/kits`. `SequencerEngine` class: pattern state (toggle/clear/add/remove rows), Web Audio lookahead scheduling, sample loading via `FileSystemFileHandle` or direct `setBuffer()`, RAF-driven playhead. Persists pattern + BPM in `kit-builder-seq` localStorage |
