...
- **Thin `web/` test coverage** — vitest is wired up (`web/src/lib/*.test.ts`, `just web-test`,
  `just check`). All of `web/src/lib/` now has tests except `pyodide.ts`. `softDelete.ts`,
  `cardStore.ts`, `kitXml.ts`, `padSounds.ts`, `patchAudio.ts`, `audioVisualizer.ts` are at
  100% lines; `songAudio.ts` 100% lines/98% funcs; `analytics.ts` 76.9% and `homeAudio.ts` 65.2%
  lines have real gaps left (untouched as of 2026-09-04). `pyodide.ts` is still untested — needs
  a real Pyodide/WASM runtime for meaningful coverage, not a DOM mock; flagged for the user to
  decide (thin marshal-shape test vs. skip) rather than attempted. ~9,000 lines of Svelte still
