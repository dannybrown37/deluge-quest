...
class FakeAudioContext {
  ...
  createOscillator() {
    return makeOscillatorNode();
  }
  createGain() {
    return makeGainNode();
  }
  createBiquadFilter() {
    return makeBiquadFilterNode();
  }
  createBufferSource() {
    return makeBufferSourceNode();
  }
  createBuffer(channels: number, length: number) {
    return makeAudioBuffer(channels, length);
  }
  createDelay() {
    return makeDelayNode();
  }
  createConvolver() {
    return makeConvolverNode();
  }
  close() {
    this.state = "closed";
  }
...
describe("playPreview / stopPreview / isPlaying", () => {
  ...
  it("applies an lfo1-to-pitch patch cable", () => {
    const patch = makePatch({
      patchCables: [
        { source: "lfo1", destination: "pitch", amount: "7FFFFFFF" },
      ],
    });
    expect(() => playPreview(patch)).not.toThrow();
  });
