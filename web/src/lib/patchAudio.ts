...
const ARP_PATTERNS: Record<
  string,
  (notes: number[], octaves: number) => number[]
> = {
  up: (notes, oct) => {
    const seq: number[] = [];
  ...
  down: (notes, oct) => {
    ...
    for (let o = oct - 1; o >= 0; o--)
      for (const n of [...notes].reverse()) seq.push(n + o * 12);
    return seq;
  },
  upDown: (notes, oct) => {
...
export function playPreview(patch: AudioPatch, onStop?: () => void) {
  ...
  const arpSeq = isArp
    ? ARP_PATTERNS[patch.arpMode](transposedNotes, patch.arpOctaves)
    : null;
  ...
  const totalDuration = arpSeq
    ? Math.min(4, arpSeq.length * 0.2 + r1 + 0.5)
    : Math.min(5, a1 + d1 + 1.5 + r1 + 0.5);
  ...
  function playNote(midi: number, startTime: number, duration: number) {
    ...
    if (env2Cable) {
      ...
      lpf.frequency.linearRampToValueAtTime(
        Math.min(20000, baseFreq + env2Amt),
        startTime + a2,
      );
      lpf.frequency.linearRampToValueAtTime(
        Math.min(20000, baseFreq + env2Amt * s2),
        startTime + a2 + d2,
      );
    }
    ...
    for (let u = 0; u < unisonCount; u++) {
      const detuneOffset =
        unisonCount === 1 ? 0 : (u / (unisonCount - 1) - 0.5) * detuneCents;
...
export function createSubVoice(
  ...
  oscA.frequency.value =
    freq *
    Math.pow(2, patch.osc1.transpose / 12) *
    Math.pow(2, patch.osc1.cents / 1200);
  ...
  if (oscBVol > 0.01) {
    ...
    oscB.frequency.value =
      freq *
      Math.pow(2, patch.osc2.transpose / 12) *
      Math.pow(2, patch.osc2.cents / 1200);
  ...
  const noiseVol = patch.params.noiseVolume
...
export function createFMVoice(
  ...
  const mod1Amt =
    (patch.params.modulator1Amount
      ? hexToNorm(patch.params.modulator1Amount)
      : 0) *
    carrierFreq *
    5;
  const mod2Amt =
    (patch.params.modulator2Amount
      ? hexToNorm(patch.params.modulator2Amount)
      : 0) *
    carrierFreq *
    5;

  const mod1Ratio = patch.modulator1
    ? Math.pow(2, patch.modulator1.transpose / 12)
    : 1;
  const mod2Ratio = patch.modulator2
    ? Math.pow(2, patch.modulator2.transpose / 12)
    : 1;
