...
describe('generateKitXml', () => {
  ...
  it('emits one <sound> per row, in order', () => {
    ...
    const xml = generateKitXml(kit);
    const doc = new DOMParser().parseFromString(xml, 'text/xml');
    const names = [...doc.querySelectorAll('sound')].map((s) => s.getAttribute('name'));
  ...
  it('escapes XML-special characters in name and samplePath', () => {
    ...
    expect(sound.querySelector('osc1')!.getAttribute('fileName')).toBe('SAMPLES/a&b.wav');
  });
});

describe('parseKitXml', () => {
  it('throws when there is no <kit> root element', () => {
    expect(() => parseKitXml('<notakit></notakit>')).toThrow('No <kit> root element found');
  });
...
describe('generateKitXml -> parseKitXml round trip', () => {
  it('preserves row data through a full save/load cycle', () => {
    const rows: KitRow[] = [
      { name: 'KICK', samplePath: 'SAMPLES/kick.wav', volume: 90, pan: -20, loopMode: 'once', polyphonic: 'mono' },
      { name: 'SNARE & CLAP', samplePath: 'SAMPLES/DRUMS/sn "1".wav', volume: 0, pan: 50, loopMode: 'loop', polyphonic: 'poly' },
      { name: 'HAT', samplePath: 'SAMPLES/hat.wav', volume: 100, pan: 0, loopMode: 'cut', polyphonic: 'choke' },
    ];
