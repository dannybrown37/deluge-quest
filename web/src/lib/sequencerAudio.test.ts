...
import { SequencerEngine, NUM_STEPS, type SequencerEngineOptions } from './sequencerAudio';
...
function makeFileHandle(arrayBuffer: () => Promise<ArrayBuffer>): FileSystemFileHandle {
  return {
...
describe('scheduling', () => {
  ...
  it('the schedule timer ticks scheduleChunk every SCHEDULE_INTERVAL_MS', () => {
    ...
    e.play();
    const ctx = currentFakeCtx();
    const spy = vi.spyOn(e as unknown as { scheduleChunk: () => void }, 'scheduleChunk' as never);
...
describe('sample loading', () => {
  it('loadSample decodes audio into sampleBuffers[row]', async () => {
    ...
    vi.stubGlobal('AudioContext', function AudioContextStub(this: FakeAudioContext) {
      const c = new FakeAudioContext();
  ...
  it('a failed decode leaves the row silent and does not throw', async () => {
    const e = makeEngine();
    e.addRow();
    const handle = makeFileHandle(async () => { throw new Error('bad file'); });
  ...
  it('a decodeAudioData rejection leaves the row silent and does not throw', async () => {
    ...
    vi.stubGlobal('AudioContext', function AudioContextStub() {
      const c = new FakeAudioContext();
      c.decodeAudioData = vi.fn(async () => { throw new Error('cannot decode'); });
      contexts.push(c);
      return c;
    });
