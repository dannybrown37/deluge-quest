...
import { SongPlayer, type SongPlaybackOptions, type EQBand } from './songAudio';
import type { PreviewTrack, PreviewPatch, PreviewNoteRow } from './pyodide';

function createParam(initial = 0) {
  return {
    value: initial,
    setValueAtTime: vi.fn(function (this: { value: number }, v: number) { this.value = v; }),
    exponentialRampToValueAtTime: vi.fn(function (this: { value: number }, v: number) { this.value = v; }),
...
function makeGainNode() {
  return { gain: createParam(1), connect: vi.fn((d: unknown) => d), disconnect: vi.fn() };
}
...
function makeBufferSourceNode() {
  return { buffer: null as unknown, connect: vi.fn((d: unknown) => d), disconnect: vi.fn(), start: vi.fn(), stop: vi.fn() };
}
function makeCompressorNode() {
  return { threshold: createParam(-24), ratio: createParam(12), connect: vi.fn((d: unknown) => d) };
}
...
class FakeAudioContext {
  ...
  createBuffer = vi.fn((channels: number, length: number) => makeAudioBuffer(channels, length));
  createDynamicsCompressor = vi.fn(() => makeCompressorNode());
  resume = vi.fn(() => { this.state = 'running'; });
  suspend = vi.fn(() => { this.state = 'suspended'; });
...
function makeClip(overrides: Partial<PreviewTrack['clips'][number]> = {}): PreviewTrack['clips'][number] {
  return {
    positionTicks: 0,
    lengthTicks: 96,
    clipLengthTicks: 96,
...
function makeOpts(overrides: Partial<SongPlaybackOptions> = {}): SongPlaybackOptions {
  return { bpm: 120, ticksPerQuarter: 48, tracks: [], durationTicks: 480, ...overrides };
}
...
describe('flattenNotes (via constructor)', () => {
  it('flattens a single note with no looping', () => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ drumName: 'KICK', notes: [{ pos: 0, len: 12, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
    const notes = internals(p).flatNotes;
    expect(notes).toHaveLength(1);
    expect(notes[0]).toMatchObject({ drumType: 'kick', startSec: 0, vel: 100 / 127 });
  });
  ...
  it('repeats notes across clip-instance loops and truncates the final partial loop', () => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({
        lengthTicks: 90, clipLengthTicks: 24,
        noteRows: [makeNoteRow({ drumName: 'SNARE', notes: [{ pos: 0, len: 4, vel: 80 }] })],
      })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
    const notes = internals(p).flatNotes;
    ...
    expect(notes.map((n) => n.startSec)).toEqual([0, 24, 48, 72].map((t) => t * internals(p).secPerTick));
  });

  it('defaults note-row midi to 60 when y is null', () => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ y: null, drumName: 'PERC', notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
  ...
  it.each([
    ...
    ['HIHAT', 'hihat'], ['HH', 'hihat'],
    ['CLAP', 'clap'], ['HAND CLAP', 'clap'],
    ['TOM', 'tom'],
    ['CRASH', 'cymbal'], ['RIDE', 'cymbal'],
    ['COWBELL', 'perc'], ['SHAKER', 'perc'],
  ] as const)('classifies drum name %s as %s', (name, expected) => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ drumName: name, notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
  ...
  it.each([
    [30, 'kick'], [45, 'snare'], [75, 'hihat'], [60, 'perc'],
  ] as const)('falls back to midi range %d -> %s when no drum name', (midi, expected) => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ y: midi, drumName: null, notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
    expect(internals(p).flatNotes[0].drumType).toBe(expected);
  });
  ...
  it('forces perc drumType for non-kit tracks regardless of midi', () => {
    const track = makeTrack({
      isKit: false,
      clips: [makeClip({ noteRows: [makeNoteRow({ y: 30, notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
  ...
  it('clamps note duration to the remaining instance length and floors it at 0.02s', () => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({
        lengthTicks: 5, clipLengthTicks: 96,
        noteRows: [makeNoteRow({ drumName: 'KICK', notes: [{ pos: 0, len: 999, vel: 100 }] })],
      })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
...
describe('play()', () => {
  it('builds the audio graph: compressor, EQ, filter, master, per-track gains, noise buffer', () => {
    ...
    expect(ctx.createBuffer).toHaveBeenCalledWith(1, ctx.sampleRate, ctx.sampleRate);
...
describe('pause/stop/seek/dispose', () => {
  ...
  it('stops and disconnects any in-flight scheduled nodes', () => {
    const p = new SongPlayer(makeOpts({ tracks: [makeTrack({ isKit: true })] }));
    p.play();
    internals(p).scheduleNote(
      { trackIdx: 0, midi: 45, isKit: true, drumType: 'kick', samplePath: null, startSec: 0, durSec: 0.1, vel: 1 },
      0
    );
...
describe('EQ and filter controls', () => {
  it.each(['low', 'mid', 'high'] as EQBand[])('getEQ/setEQ round-trips band %s', (band) => {
    const p = new SongPlayer(makeOpts({ tracks: [makeTrack()] }));
    p.setEQ(band, 3);
    expect(p.getEQ(band)).toBe(3);
  });

  it.each(['low', 'mid', 'high'] as EQBand[])('setEQ updates the live node once playing (%s)', (band) => {
    const p = new SongPlayer(makeOpts({ tracks: [makeTrack()] }));
    p.play();
...
describe('scheduleNote dispatch', () => {
  ...
  function note(overrides: Partial<NoteInternal>): NoteInternal {
    return { trackIdx: 0, midi: 60, isKit: false, drumType: 'perc', samplePath: null, startSec: 0, durSec: 0.1, vel: 1, ...overrides };
  }
  ...
  it('plays a resolved sample buffer when one is cached for the note path', () => {
    ...
    internals(p).scheduleNote(note({ isKit: true, samplePath: 'kit/x.wav' }), 0);
  ...
  it.each(['kick', 'snare', 'hihat', 'clap', 'tom', 'cymbal', 'perc'] as const)(
    ...
    (drumType) => {
      const p = playerWithTracks([makeTrack({ isKit: true })]);
      const before = internals(p).scheduled.length;
      expect(() => internals(p).scheduleNote(note({ isKit: true, drumType, midi: 45 }), 0)).not.toThrow();
  ...
  it('kick voice ramps a sine oscillator from 150Hz down toward 30Hz', () => {
    ...
    const osc = ctx.createOscillator.mock.results.at(-1)!.value as ReturnType<typeof makeOscillatorNode>;
  ...
  it('routes through the synth patch voice path for non-kit tracks with a patch', () => {
    ...
    expect(() => internals(p).scheduleNote(note({ isKit: false }), 0)).not.toThrow();
  ...
  it('routes through the FM patch voice path when patch.mode is fm', () => {
    const p = playerWithTracks([makeTrack({ patch: makePatch({ mode: 'fm' }) })]);
    expect(() => internals(p).scheduleNote(note({ isKit: false }), 0)).not.toThrow();
  });

  it('falls back to a plain oscillator for non-kit tracks with no patch', () => {
    const p = playerWithTracks([makeTrack({ patch: null }), makeTrack({ patch: null })]);
    const before = internals(p).scheduled.length;
...
describe('scheduleChunk / onEnd', () => {
  it('schedules a note that falls inside the lookahead window on play()', async () => {
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ drumName: 'KICK', notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track] }));
    p.play();
  ...
  it('stops and fires onEnd once elapsed time passes the total duration', () => {
    const onEnd = vi.fn();
    const p = new SongPlayer(makeOpts({ tracks: [makeTrack()], durationTicks: 48, onEnd }));
    p.play();
  ...
  it('fetches samples through the resolver and caches them before scheduling', async () => {
    ...
    const track = makeTrack({
      isKit: true,
      clips: [makeClip({ noteRows: [makeNoteRow({ drumName: 'KICK', samplePath: 'kit/kick.wav', notes: [{ pos: 0, len: 4, vel: 100 }] })] })],
    });
    const p = new SongPlayer(makeOpts({ tracks: [track], sampleResolver: resolver }));
    p.play();
    await Promise.resolve();
    await Promise.resolve();
    await Promise.resolve();
