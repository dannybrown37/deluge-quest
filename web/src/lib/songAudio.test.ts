...
interface SongPlayerInternals {
  ...
  preRenderedBuffers: AudioBuffer[] | null;
  preRenderedSources: {
    buffer: unknown;
    connect: unknown;
    start: unknown;
...
describe("pre-rendering", () => {
  function twoTrackOpts(): SongPlaybackOptions {
    return makeOpts({
      tracks: [
        makeTrack({
          name: "Synth1",
          ...
          clips: [
            makeClip({
              noteRows: [
                makeNoteRow({ notes: [{ pos: 0, len: 12, vel: 100 }] }),
              ],
            }),
          ],
        }),
  ...
  it("produces one AudioBuffer per track", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    expect(internals(p).preRenderedBuffers).toHaveLength(2);
    expect(offlineContexts).toHaveLength(2);
    for (const oc of offlineContexts) {
      expect(oc.startRendering).toHaveBeenCalledOnce();
    }
  });

  it("calls onProgress for each completed track", async () => {
    const progress = vi.fn();
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender(progress);
    expect(progress).toHaveBeenCalledTimes(2);
    expect(progress).toHaveBeenCalledWith(1, 2);
    expect(progress).toHaveBeenCalledWith(2, 2);
  });

  it("does not corrupt player state during rendering", async () => {
    const p = new SongPlayer(twoTrackOpts());
    p.play();
    p.stop();

    const scheduledBefore = internals(p).scheduled;
    await p.preRender();

    expect(internals(p).ctx).toBe(null);
    expect(internals(p).scheduled).toStrictEqual(scheduledBefore);
  });

  it("play after preRender creates buffer sources instead of real-time scheduling", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();

    const sources = internals(p).preRenderedSources;
  ...
  it("does not start a real-time scheduler when using pre-rendered buffers", async () => {
    vi.useFakeTimers();
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();
  ...
  it("volume changes during pre-rendered playback are instant via gain nodes", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();

    p.setTrackVolume(0, 0.5);
    const gain = internals(p).trackGains[0];
    expect(gain.gain.setTargetAtTime).toHaveBeenCalled();
  });

  it("mute during pre-rendered playback sets gain to zero", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();
    ...
    p.setTrackMuted(0, true);
    const gain = internals(p).trackGains[0];
    expect(gain.gain.setTargetAtTime).toHaveBeenCalledWith(
      0,
      expect.anything(),
      0.01,
    );
  ...
  it("seek during pre-rendered playback stops old sources and starts new ones", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();

    const oldSources = [...internals(p).preRenderedSources];
    p.seek(48);
    for (const src of oldSources) {
      expect(src.stop).toHaveBeenCalled();
    }
    const newSources = internals(p).preRenderedSources;
    expect(newSources).toHaveLength(2);
    expect(newSources).not.toEqual(oldSources);
  });

  it("pause and resume work with pre-rendered playback", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();
    expect(p.isPlaying).toBe(true);
  ...
  it("stop clears pre-rendered sources but preserves buffers for replay", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.play();
    p.stop();

    expect(internals(p).preRenderedSources).toHaveLength(0);
    expect(internals(p).preRenderedBuffers).toHaveLength(2);
  });

  it("dispose clears pre-rendered buffers", async () => {
    const p = new SongPlayer(twoTrackOpts());
    await p.preRender();
    p.dispose();

    expect(internals(p).preRenderedBuffers).toBeNull();
  });

  it("prefetches samples before offline rendering when resolver exists", async () => {
    const buf = makeAudioBuffer(1, 100);
    const resolver = vi.fn(async () => buf);
    const track = makeTrack({
      isKit: true,
