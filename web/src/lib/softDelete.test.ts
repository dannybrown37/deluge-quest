...
class FakeDirHandle {
  ...
  async getDirectoryHandle(name: string, opts?: { create?: boolean }) {
    ...
    if (!(child instanceof FakeDirHandle))
...
describe("getOrCreateDir", () => {
  it("creates nested directories", async () => {
    const root = makeRoot();
    const handle = await getOrCreateDir(
  ...
  it("reuses an existing directory rather than recreating it", async () => {
    const root = makeRoot();
    const first = await getOrCreateDir(
      root as unknown as FileSystemDirectoryHandle,
      "A",
    );
    const second = await getOrCreateDir(
      root as unknown as FileSystemDirectoryHandle,
      "A",
    );
...
describe("moveToTrash", () => {
  ...
  it("mirrors nested source directories under SOFT_DELETE", async () => {
    ...
    expect(await readFile(root, `${TRASH_DIR}/SAMPLES/DRUMS/kick.wav`)).toBe(
      "audio-bytes",
    );
  });
});
...
describe("restoreFile", () => {
  ...
  it("backs the current version up first, so nothing is destroyed", async () => {
    ...
    expect(await readFile(root, `${HISTORY_BACKUP_DIR}/SONGS/A.XML`)).toBe(
      "<song>new</song>",
    );
  });
  ...
  it("makes no backup when there was nothing to overwrite", async () => {
    ...
    expect(await fileExists(root, `${HISTORY_BACKUP_DIR}/SONGS/GONE.XML`)).toBe(
      false,
    );
  });
  ...
  it("keeps a second restore of the same file from losing the first backup", async () => {
    ...
    expect(await readFile(root, `${HISTORY_BACKUP_DIR}/SONGS/A.XML.1`)).toBe(
      "v1",
    );
  });
