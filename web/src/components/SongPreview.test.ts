...
vi.mock("../lib/songAudio", () => ({
  SongPlayer: vi.fn().mockImplementation(function (
    this: FakePlayer,
    opts: Partial<FakePlayer>,
  ) {
...
import { inspectSong, loadPyodide } from "../lib/pyodide";
...
beforeEach(() => {
  ...
  mockLoadPyodide.mockImplementation(
    async (onProgress?: (stage: string, pct: number) => void) => {
...
describe("SongPreview", () => {
  ...
  it("toggles play/pause and creates a player on first play", async () => {
    ...
    expect(
      document.querySelector('.transport-btn[title="Pause"]'),
    ).toBeTruthy();

    await fireEvent.click(
      document.querySelector('.transport-btn[title="Pause"]') as HTMLElement,
    );
  ...
  it("shows and hides a clip tooltip on hover", async () => {
    ...
    const clipRect = document.querySelector(
      '.timeline-svg rect[style*="cursor: pointer"]',
    ) as SVGRectElement;
  ...
  it("adjusts the EQ Low knob by dragging", async () => {
    ...
    expect(playerInstances[0].setEQ).toHaveBeenCalledWith(
      "low",
      expect.any(Number),
    );
  });
  ...
  it("shows fractional bar length in the tooltip", async () => {
    ...
    await fireEvent.mouseEnter(clipRect, { clientX: 100, clientY: 50 });
    const text =
      (document.querySelector(".tooltip") as HTMLElement).textContent ?? "";
  ...
  it("shows singular wording for one cached song and one stats result", async () => {
    ...
    render(SongPreview);
    await waitFor(() =>
      expect(screen.getByText("1 song loaded in Song Stats")).toBeTruthy(),
    );
    await waitFor(() =>
      expect(
        screen.getByText(/1 song with arrangement data on CARD/),
      ).toBeTruthy(),
    );
  ...
  it("resets playing state and uses card sample resolver when card is loaded", async () => {
    ...
    const resolver = (
      playerInstances[0] as unknown as {
        sampleResolver: (p: string, c: unknown) => unknown;
      }
    ).sampleResolver;
    resolver("sample.wav", {});
    expect(
      (cardStore as unknown as { getSampleBuffer: ReturnType<typeof vi.fn> })
        .getSampleBuffer,
  ...
  it("scales ruler step marks for very long songs", async () => {
    ...
    const rulerLabels = Array.from(
      document.querySelectorAll(".ruler-text"),
    ).map((t) => t.textContent);
  ...
  it("falls back to a generic error message when inspection throws without one", async () => {
    ...
    await fireEvent.drop(dropzone, dropEvent(xmlFile("song.XML")));
    await waitFor(() =>
      expect(screen.getByText("Failed to parse song")).toBeTruthy(),
    );
  });
        ...
        tracks: [
          {
            name: "Kit Track",
            isKit: true,
            instrumentType: undefined as unknown as
              | "synth"
              | "kit"
              | "midi"
              | "cv"
  ...
  it("shows plural songs and no card name when the card has no root handle", async () => {
    ...
    const title =
      document.querySelector(".card-picker-title")?.textContent ?? "";
