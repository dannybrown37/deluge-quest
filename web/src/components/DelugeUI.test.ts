...
vi.mock("../lib/homeAudio", () => ({
  homeAudio: {
    ...
    loadedSongIndex: -1,
    currentSongIndex: 0,
    songs: [{ name: "First Song" }, { name: "Second Song" }] as {
      name: string;
      duration?: string;
      genre?: string;
      year?: string | number;
...
const oled = (c: HTMLElement) =>
  c.querySelector(".oled-text")?.textContent?.trim();
const sub = (c: HTMLElement) =>
  c.querySelector(".oled-subtext")?.textContent?.trim();
...
describe("DelugeUI", () => {
  ...
  describe("screen", () => {
    ...
    it('re-derives "drop a song to begin" from idleSubtext when the pointer leaves with no songs', async () => {
      ...
      await vi.waitFor(() =>
        expect(sub(container)).toBe("drop a song to begin"),
      );
    });
    ...
    it("scrolls the marquee when the song name overflows the screen", async () => {
      ...
      Object.defineProperty(oledText, "clientWidth", {
        value: 100,
        configurable: true,
      });
      await fireEvent.mouseEnter(screen.getByLabelText("Card Management"));
      await vi.waitFor(() =>
        expect(oledText.classList.contains("is-scrolling")).toBe(true),
      );
    });
  ...
  describe("transport", () => {
    ...
    it("reports the paused position after stopping mid-song", async () => {
      ...
      await fireEvent.click(screen.getByLabelText("Play"));
      await vi.waitFor(() =>
        expect(sub(container)).toBe("paused · 0:00 / 1:40"),
      );
    });
  });
  ...
  describe("song browser", () => {
    ...
    it("moves the cursor on mouse hover over a song option", async () => {
      ...
      await fireEvent.click(screen.getByLabelText("Load song"));
      const option = screen
        .getByText("Second Song")
        .closest(".song-option") as HTMLElement;
  ...
  describe("knobs", () => {
    ...
    it("routes each knob to its own audio parameter", async () => {
      render(DelugeUI);
      await fireEvent.wheel(screen.getByLabelText("delay time"), {
        deltaY: -100,
      });
    ...
    it("changes the value on a mouse drag", async () => {
      const { container } = render(DelugeUI);
      await fireEvent.mouseDown(screen.getByLabelText("output"), {
        clientY: 100,
      });
