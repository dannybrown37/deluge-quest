...
function hex(value: number): string {
  const clamped = Math.max(
    -2147483648,
    Math.min(2147483647, Math.round(value)),
  );
...
function generatePatch(cat: Category): Patch {
  const mode: SynthMode =
    cat === "keys"
      ? weightedPick(["subtractive", "fm"], [0.4, 0.6])
      : cat === "bass"
        ? weightedPick(["subtractive", "fm"], [0.7, 0.3])
  ...
  const polyphonic =
    cat === "lead"
      ? weightedPick(POLY_MODES, [0.2, 0.5, 0.3, 0])
      : cat === "bass"
        ? weightedPick(POLY_MODES, [0.3, 0.5, 0.2, 0])
        : cat === "pad"
          ? "poly"
          : cat === "fx"
            ? pick(["poly", "mono"])
  ...
  const modFXType =
    cat === "pad"
      ? weightedPick(MOD_FX_TYPES, [0.2, 0.3, 0.3, 0.2])
      : cat === "fx"
        ? weightedPick(MOD_FX_TYPES, [0.1, 0.3, 0.2, 0.4])
        : weightedPick(MOD_FX_TYPES, [0.5, 0.2, 0.2, 0.1]);

  const arpMode =
    cat === "lead"
      ? weightedPick(ARP_MODES, [0.5, 0.15, 0.1, 0.15, 0.1])
      : cat === "keys"
        ? weightedPick(ARP_MODES, [0.4, 0.2, 0.1, 0.2, 0.1])
        : "off";
  ...
  const params: Record<string, string> = {
    ...
    portamento:
      polyphonic === "legato"
        ? randHexFloat(0x00000000, 0x40000000)
        : hex(-0x80000000),
    ...
    noiseVolume:
      cat === "fx" ? randHexFloat(-0x60000000, 0x00000000) : hex(-0x80000000),
    ...
    modulator1Amount: isFM
      ? randHexFloat(-0x20000000, 0x60000000)
      : hex(-0x80000000),
    modulator1Feedback: isFM
      ? randHexFloat(-0x80000000, 0x00000000)
      : hex(-0x80000000),
    modulator2Amount: isFM
      ? randHexFloat(-0x20000000, 0x60000000)
      : hex(-0x80000000),
    modulator2Feedback: isFM
      ? randHexFloat(-0x80000000, 0x00000000)
      : hex(-0x80000000),
    carrier1Feedback: isFM
      ? randHexFloat(-0x80000000, 0x00000000)
      : hex(-0x80000000),
    carrier2Feedback: isFM
      ? randHexFloat(-0x80000000, 0x00000000)
      : hex(-0x80000000),
    modFXRate:
      modFXType !== "none" ? randHexFloat(-0x20000000, 0x20000000) : hex(0),
    modFXDepth:
      modFXType !== "none" ? randHexFloat(0x00000000, 0x40000000) : hex(0),
    delayRate: hex(0),
    delayFeedback:
      Math.random() > 0.6
        ? randHexFloat(-0x40000000, 0x20000000)
        : hex(-0x80000000),
    reverbAmount:
      Math.random() > 0.5
        ? randHexFloat(-0x40000000, 0x20000000)
    ...
    bitCrush:
      cat === "fx" && Math.random() > 0.7
        ? randHexFloat(-0x40000000, 0x00000000)
        : hex(-0x80000000),
    modFXOffset: hex(0),
    modFXFeedback:
      modFXType !== "none" ? randHexFloat(-0x40000000, 0x20000000) : hex(0),
  };
  ...
  if (cat === "pad" || cat === "fx") {
    if (Math.random() > 0.4) {
      patchCables.push({
        source: "lfo1",
        destination: "pitch",
        amount: randHexFloat(0x02000000, 0x10000000),
      });
    }
  }
  if (!isFM && Math.random() > 0.5) {
    patchCables.push({
      ...
      amount: randHexFloat(0x10000000, 0x50000000),
    });
  }
...
function toXML(p: Patch): string {
  ...
  const modKnobs = (mp || [])
    .map((param, i) => {
      let extra = "";
      if (param === "volumePostReverbSend")
        extra = ' patchAmountFromSource="compressor"';
      if (param === "pitch" && i === 10)
        extra = ' patchAmountFromSource="lfo1"';
      return `\t\t<modKnob controlsParam="${param}"${extra} />`;
    })
    .join("\n");

  const cables = p.patchCables
    .map(
      (c) =>
  ...
  const paramAttrs = Object.entries(p.params)
    .map(([k, v]) => `\t\t${k}="${v}"`)
    .join("\n");
...
function describeOsc(osc: OscConfig): string {
  ...
  if (osc.transpose !== 0)
    s += ` ${osc.transpose > 0 ? "+" : ""}${osc.transpose}st`;
...
function getDisplayParams(p: Patch): ParamDisplay[] {
  const items: ParamDisplay[] = [
    { label: "Volume", key: "volume", value: hexToPercent(p.params.volume) },
    {
      label: "LPF Freq",
      key: "lpfFrequency",
      value: hexToPercent(p.params.lpfFrequency),
    },
    {
      label: "LPF Res",
      key: "lpfResonance",
      value: hexToPercent(p.params.lpfResonance),
    },
    {
      label: "Osc B Vol",
      key: "oscBVolume",
      value: hexToPercent(p.params.oscBVolume),
    },
    {
      label: "LFO1 Rate",
      key: "lfo1Rate",
      value: hexToPercent(p.params.lfo1Rate),
    },
    {
      label: "Reverb",
      key: "reverbAmount",
      value: hexToPercent(p.params.reverbAmount),
    },
    {
      label: "Delay FB",
      key: "delayFeedback",
      value: hexToPercent(p.params.delayFeedback),
    },
  ];
  if (p.mode === "fm") {
    items.push(
      {
        label: "Mod1 Amt",
        key: "modulator1Amount",
        value: hexToPercent(p.params.modulator1Amount),
      },
      {
        label: "Mod2 Amt",
        key: "modulator2Amount",
        value: hexToPercent(p.params.modulator2Amount),
      },
    );
  }
  if (p.params.noiseVolume !== "0x80000000") {
    items.push({
      label: "Noise",
      key: "noiseVolume",
      value: hexToPercent(p.params.noiseVolume),
    });
  }
  return items;
}

function envDisplay(env: EnvelopeConfig): {
  a: number;
  d: number;
  s: number;
  r: number;
...
function togglePreview() {
  if (playing) {
    ...
    playPreview(patch as unknown as AudioPatch, () => {
      playing = false;
    });
  }
}
...
let e1 = $derived(
  patch ? envDisplay(patch.envelope1) : { a: 0, d: 0, s: 0, r: 0 },
);
let e2 = $derived(
  patch ? envDisplay(patch.envelope2) : { a: 0, d: 0, s: 0, r: 0 },
);
