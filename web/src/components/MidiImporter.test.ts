...
const mockConvert = convertMidiToDelugeXml as unknown as ReturnType<typeof vi.fn>;
...
describe('MidiImporter', () => {
  ...
  it('resets from the done state via "Import another"', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: 'Import another' }));
    expect(screen.getByText('Drop a MIDI file')).toBeTruthy();
  });
  ...
  it('downloads the result XML', async () => {
    ...
    await fireEvent.click(screen.getByRole('button', { name: 'Download Deluge XML' }));
