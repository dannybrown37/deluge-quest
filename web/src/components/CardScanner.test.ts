  ...
  return {
    ...
    cardStore: {
      ...
      sampleIndex: new Map<
        string,
        {
          handle: { getFile: ReturnType<typeof vi.fn> };
          size: number;
          path: string;
        }
      >(),
...
import { getOrCreateDir, moveToTrash } from "../lib/softDelete";
...
const mockGetOrCreateDir = getOrCreateDir as unknown as ReturnType<
  typeof vi.fn
>;
...
function seedCard() {
  ...
  mockCardStore.sampleIndex.set(
    "samples/kick.wav",
    fakeSample("SAMPLES/kick.wav", "kick-data-unique"),
  );
  mockCardStore.sampleIndex.set(
    "samples/unused.wav",
    fakeSample("SAMPLES/unused.wav", "unused-data"),
  );
  mockCardStore.sampleIndex.set(
    "samples/dup1.wav",
    fakeSample("SAMPLES/dup1.wav", "duplicate-bytes-xyz"),
  );
  mockCardStore.sampleIndex.set(
    "samples/dup2.wav",
    fakeSample("SAMPLES/dup2.wav", "duplicate-bytes-xyz"),
  );
...
describe("CardScanner", () => {
  ...
  it("scans a card and reports sample/reference/unused/broken-ref counts", async () => {
    ...
    const stats = Array.from(document.querySelectorAll(".stat-value")).map(
      (n) => n.textContent,
    );
  ...
  it("matches sample refs case-insensitively (FAT32)", async () => {
    ...
    mockCardStore.sampleIndex.set(
      ...
      fakeSample("SAMPLES/Drums/kick.wav", "k"),
    );
    mockCardStore.songXmls = new Map();
    mockCardStore.songXmls.set(
      "SONGS/s.XML",
      '<song><osc1 fileName="SAMPLES/DRUMS/KICK.WAV" /></song>',
    );
    mockCardStore.presetIndex = new Map();
    ...
    await fireEvent.click(screen.getByText("Browse for folder"));
    await waitFor(
      () => expect(screen.getByText("Sample Library (1)")).toBeTruthy(),
      { timeout: 3000 },
    );
  ...
  it("strips leading slashes from XML refs", async () => {
    ...
    mockCardStore.sampleIndex.set(
      "samples/hit.wav",
      fakeSample("SAMPLES/HIT.WAV", "h"),
    );
    mockCardStore.songXmls = new Map();
    mockCardStore.songXmls.set(
      "SONGS/s.XML",
      '<song><osc1 fileName="/SAMPLES/HIT.WAV" /></song>',
    );
    mockCardStore.presetIndex = new Map();
  ...
  it("deletes an unused sample from the analysis tab", async () => {
    ...
    await scanAndWait();
    await fireEvent.click(screen.getByText("Analysis"));
    const unusedSection = screen
      .getByText(/Unused samples/)
      .closest(".analysis-section") as HTMLElement;
    await fireEvent.click(within(unusedSection).getByText("SAMPLES/"));

    const unusedRow = screen
      .getByText("unused.wav")
      .closest(".tree-file") as HTMLElement;
  ...
  it("offers reconnect on the dropzone when a persisted handle exists", async () => {
    ...
    await waitFor(() =>
      expect(
        screen.getByText("Reconnect previously loaded SD card"),
      ).toBeTruthy(),
    );
  });
  ...
  it("scans via drag-and-drop of a file list", async () => {
    ...
    Object.defineProperty(file, "webkitRelativePath", {
      value: "CARD/SAMPLES/kick.wav",
    });

    render(CardScanner);
    const dropzone = document.querySelector(".dropzone") as HTMLElement;
    await fireEvent.drop(dropzone, {
      dataTransfer: { files: [file], items: undefined },
    });
  ...
  it("clicking the unused stat jumps to the analysis tab", async () => {
    ...
    await fireEvent.click(
      screen.getByText("unused").closest("button") as HTMLElement,
    );
  ...
  it("clicking the broken refs stat jumps to samples and expands broken refs", async () => {
    await scanAndWait();
    await fireEvent.click(
      screen.getByText("broken refs").closest("button") as HTMLElement,
    );
  ...
  it("fixes an auto-fixable invalid XML file, backing up the original", async () => {
    ...
    mockGetOrCreateDir.mockImplementation(
      async (_root: unknown, path: string) =>
  ...
  it('fixes all auto-fixable XML files via "Fix all"', async () => {
    ...
    const backupDir = fakeWritableDir();
    const liveDir = fakeWritableDir({
      "badkit.XML": BAD_KIT_XML,
      "badkit2.XML": BAD_KIT_XML,
    });
    mockGetOrCreateDir.mockImplementation(
      async (_root: unknown, path: string) =>
        path.startsWith("REPAIR_BACKUP") ? backupDir : liveDir,
    ...
    await waitFor(() =>
      expect(mockTrack).toHaveBeenCalledWith("manage", "fix_all_xml"),
    );
  ...
  it('shows "no issues found" after scanning duplicates with nothing flagged', async () => {
    ...
    mockCardStore.sampleIndex.set(
      "samples/kick.wav",
      fakeSample("SAMPLES/kick.wav", "k"),
    );
    mockCardStore.songXmls = new Map();
    mockCardStore.songXmls.set(
      "SONGS/s.XML",
      '<song><osc1 fileName="SAMPLES/KICK.WAV" /></song>',
    );
    mockCardStore.presetIndex = new Map();
  ...
  it('reports "already sorted" when a song is moved to the category it is already in', async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("already sorted")).toBeTruthy(),
    );
  });
  ...
  it("does not show Sort/Move controls on the songs tab after a drag-and-drop scan", async () => {
    ...
    Object.defineProperty(songFile, "webkitRelativePath", {
      value: "CARD/SONGS/song1.xml",
    });

    render(CardScanner);
    const dropzone = document.querySelector(".dropzone") as HTMLElement;
    await fireEvent.drop(dropzone, {
      dataTransfer: { files: [file, songFile], items: undefined },
    });
    await waitFor(
      () => expect(screen.getByText("Sample Library (1)")).toBeTruthy(),
      { timeout: 3000 },
  ...
  it("shows sample sizes in kilobytes and megabytes when large enough", async () => {
    ...
    mockCardStore.sampleIndex.set(
      "samples/small.wav",
      fakeSample("SAMPLES/small.wav", "x".repeat(2048)),
    );
    mockCardStore.sampleIndex.set(
      "samples/big.wav",
      fakeSample("SAMPLES/big.wav", "y".repeat(2 * 1024 * 1024)),
    );
    mockCardStore.songXmls = new Map();
    mockCardStore.presetIndex = new Map();
    ...
    await fireEvent.click(screen.getByText("Browse for folder"));
    await waitFor(
      () => expect(screen.getByText("Sample Library (2)")).toBeTruthy(),
      { timeout: 3000 },
    );
    await expandSamplesFolder();
  ...
  it("moves an external-gear song and shows its destination path", async () => {
    ...
    await waitFor(() =>
      expect(within(externalRow).getByText(/→/)).toBeTruthy(),
    );
  });
  ...
  it("does not show a delete button for unused samples after a drag-and-drop scan", async () => {
    ...
    Object.defineProperty(kick, "webkitRelativePath", {
      value: "CARD/SAMPLES/kick.wav",
    });
    const unused = new File(["unused-data"], "unused.wav");
    Object.defineProperty(unused, "webkitRelativePath", {
      value: "CARD/SAMPLES/unused.wav",
    });

    render(CardScanner);
    const dropzone = document.querySelector(".dropzone") as HTMLElement;
    await fireEvent.drop(dropzone, {
      dataTransfer: { files: [kick, unused], items: undefined },
    });
    await waitFor(
      () => expect(screen.getByText("Sample Library (2)")).toBeTruthy(),
      { timeout: 3000 },
